...
x_image = tf.reshape(x, [-1, 28, 28, 1])#input shape [ batcg,w , h , channel]
pad1 = 'same'
h_conv1 = tf.nn.relu(conv2d(x_image, W_conv1,pad1) + b_conv1) # Conv2d = conv2d(layersize , weight) +bias

h_pool1 = max_pool_2x2(h_conv1)
W_conv2 = weight_variable([5, 5, 32, 64])# 32 feature 5X5 patch 1 channels
b_conv2 = bias_variable([64])#output channels
pad2 = 'same'
h_conv2 = tf.nn.relu(conv2d(h_pool1, W_conv2,pad2) + b_conv2)# = conv2d(layersize , weight) +bias

h_pool2 = max_pool_2x2(h_conv2)
W_fc1 = weight_variable([7 * 7 * 64, 1024])# 32 feature 5X5 patch 1 channels
b_fc1 = bias_variable([1024])#output channels
h_pool2_flat = tf.reshape(h_pool2, [-1, 7*7*64]) #Dense
h_fc1 = tf.nn.relu(tf.matmul(h_pool2_flat, W_fc1) + b_fc1)# = conv2d(layersize , weight) +bias
...
y_conv = tf.matmul(h_fc1_drop, W_fc2) + b_fc2 #result
